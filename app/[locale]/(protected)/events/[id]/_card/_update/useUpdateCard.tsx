...
const updateEventCardSchema = z.object({
  guestName: z.string()
...
export const useUpdateCard = (eventId: string, detail: EventCardSearchItemDto, onClose: () => void) => {
  const t = useTranslations();
  const { mutateAsync } = useUpdateEventCard();
  const form = useForm<UpdateEventCardRequest>({
    resolver: zodResolver(updateEventCardSchema),
    defaultValues: {
